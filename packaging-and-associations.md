...
- Clean native macOS (no iPhone-on-Mac / Catalyst). Uses platform native UI via BnsAppBar (minimal elevation AppBar).
...
- Build: `flutter build macos --release`. The app feels native on macOS. Apple Silicon friendly.
- File handling: On open, use `BnsFileHandler` (desktop args work on macOS). Self-contained, no extra installs.
