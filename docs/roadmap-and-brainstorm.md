...
2026-07-08 wave 10 — **parts, quarters, consent, and level 3 wide open** (owner spec, informed by his grandfather's Alzheimer's — "he is a puppet, they engineered him to work ok with the dog… whatever he remembers for a second is gold"):
- ✅ **RoutineStep entity** — "an entity for each part of the routine, where we put the information": each part has a title + helping note, ordered; foolproof editor rows in the routine form (numbered, removable, empty ones dropped); Today tiles show "Next: <part> (2 of 4)" with the note and a quiet **Part ✓** button; last part flows into the normal gentle "Is it done?". Per-day progress lives in the local store (`stepProgress`), resets on uncheck, pruned with retention. Cross-device step progress: parked (finished days travel via logs).
- ✅ **QUARTER-HOUR LAW** — "time isn't 2:07, never": every entered time snaps to :00/:15/:30/:45 (routine time picker with rounding notice; event add). No ugly numbers anywhere.
- ✅ **Consent over notes** — checking done on a routine that has a `need-help` note SHOWS the note ("You wrote about this one… the note stays kept either way. Done anyway?") — accepting stops today's reminding, never deletes the words.
- ✅ **Foolproof routine add** — cannot save broken: weekly/custom with zero days auto-heals to daily with a notice; title required; steps sanitized.
- ✅ **Order option** — default stays morning→night; optional "what's next": at 18:18 the 18:30 task is on top (upcoming by nearness, then earlier-today, then timeless; handled items sink). One-tap toggle on Today, persisted (`todayOrder`).
