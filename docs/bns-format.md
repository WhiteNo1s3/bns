...
- The **live database** is an open structure: `bns_data.json` (+ `audio/` and `exports/` folders) in the **BNS home**. Every change is written **immediately and atomically** — there is no save button, no save-on-exit dependency, nothing for the user to remember. A crash or battery death at any moment loses at most the keystroke in flight, never the day.
- The **home is chosen, not hardcoded** (2026-08-09): a tiny pointer file `bns_home.txt` in the platform documents folder names the folder where everything lives. No pointer (or a dead one — unplugged drive) = the documents folder, as before. Moving home copies data + audio + exports and leaves the old folder untouched as a backup (`lib/data/local/bns_home.dart`, `IsarService.moveHome`). Desktop UI: Sync screen → "Where your BNS lives". Audio references in captures are absolute paths from whichever machine recorded them; every reader goes through `IsarService.resolveAudioPath`, which falls back to the same filename inside the current `audio/` folder — so voice notes survive machine changes, imports, and home moves.
